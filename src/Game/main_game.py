...
# Initialize score at module level
score = 0
...
class Pipe(pygame.sprite.Sprite):
    def __init__(self, inverted, xpos, ysize):
        ...
        if inverted:
            ...
            self.passed = False  # For score tracking
        self.mask = pygame.mask.from_surface(self.image)
...
def main():
    """
    Main game loop for the manual Flappy Bird game.
    Handles start screen, gameplay, collision detection, and game-over state.
    """
    global score  # Use the module-level 'score' variable
    reset_game()
    clock = pygame.time.Clock()
    state = 'start'  # states: 'start', 'playing', 'game_over'
    font = pygame.font.Font(None, 36)
